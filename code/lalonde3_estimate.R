...
band <- out1[1, 3:4]
est <- out1[1, 1]
plot_coef(out4, band = band, line = est, ylim = ylim, main = "(A) NSW-CPS1")
graphics.off()

pdf("graphs/lalonde/coef_nsw_psid.pdf", width = 12, height = 3)
band <- out1[1, 3:4]
est <- out1[1, 1]
plot_coef(out5, band = band, line = est, ylim = ylim, main = "(B) NSW-PSID1")
graphics.off()
...
band <- out2[1, 3:4]
est <- out2[1, 1]
plot_coef(out6, band = band, line = est, ylim = ylim, main = "(C) Trimmed NSW-CPS1")
graphics.off()
...
band <- out3[1, 3:4]
est <- out3[1, 1]
plot_coef(out7, band = band, line = est, ylim = ylim, main = "(D) Trimmed NSW-PSID1")
graphics.off()
